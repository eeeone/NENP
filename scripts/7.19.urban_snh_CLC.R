...
location_eu <- location %>% 
  filter(!(study_ID %in% c(654,1153)))
tif1 <- rast("data/data_extraction/urban_snh_extraction/CLCplus_2018_010m_1.tif")
tif2 <- rast("data/data_extraction/urban_snh_extraction/CLCplus_2018_010m_2.tif")
tif <- merge(tif1, tif2)
pollinator_snh_codes <- c(2, 3, 4, 5, 6)
points_sf <- st_as_sf(location_eu, coords = c("longitude", "latitude"), crs = 4326)
points_proj <- st_transform(points_sf, crs = crs(tif1))
